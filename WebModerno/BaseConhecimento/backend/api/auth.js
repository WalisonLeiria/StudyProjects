...
module.exports = app => {
  const signIn = async (request, response) => {
    ...
    const user = await app.db("users").where({ email: request.body.email }).whereNull("deletedAt").first();
    if (!user) return response.status(400).send("Usuário e senha inválidos!");
